---
title: "Release 2025.09 | White Label | Make Developer Hub"
url: https://developers.make.com/white-label-documentation/release-notes/release-2025.09
scraped_at: 2026-04-21T12:42:13.570197Z
---

1. Release notes

# Release 2025.09

## hashtag Current software version numbers

The following is a list of current software versions running in Make's release environment. You can also find announcements of planned updates and upcoming end-of-life support for specific versions here.

### hashtag Containerization

Kubernetes

1.33

-

### hashtag Databases

PostgreSQL

15.12

-

Redis

v6.2.20

-

MongoDB Cloud

7.0

-

ElasticSearch

7.17.15

-

### hashtag Message Queues

RabbitMQ

3.13.7.1

-

Erlang

26.2.5.11

-

### hashtag Filesystem

NFS

4.1

-

The following are the current version numbers for services. You can verify them in your instance by going to Administration > Monitoring .

accman

```
accman
```

2ef07ff040c557917c474bf5c0546cbecd0dd335

Yes

apps-processor

```
apps-processor
```

v2.4.3

-

agency

```
agency
```

4.0-beta

-

aws-rds-log-reader

```
aws-rds-log-reader
```

v1.1.1

-

broker

```
broker
```

da019774c6f3256f0fa227e35f1e2b0b6b541b75

Yes

broker-gw-logger

```
broker-gw-logger
```

da019774c6f3256f0fa227e35f1e2b0b6b541b75

Yes

cron

```
cron
```

v1.1.1

-

datadog-agent

```
datadog-agent
```

7.71.1

-

datadog-cluster-agent

```
datadog-cluster-agent
```

7.71.1

-

db-updater

```
db-updater
```

eecd37967f7defbbde9ba5041b3a2909ed191e11

Yes

emails-processor

```
emails-processor
```

d494caac00faa491fd7a3d4c39692bef16560a69

Yes

engine

```
engine
```

35b2ef7-20251121

Yes

gateway

```
gateway
```

2e7b7af9bdaa8b3866a324badb30f310285ae3e8

Yes

imt-auditman

```
imt-auditman
```

1.11.2

Yes

ipm-server

```
ipm-server
```

3.52.0

-

ipm-service

```
ipm-service
```

2.1.2

-

kibana

```
kibana
```

7.17.15

-

lickman

```
lickman
```

085e06276ca2f17651af840870ac92d3a9323e4b

Yes

make-apps-processor

```
make-apps-processor
```

1.5.1

-

mongo-auto-indexer

```
mongo-auto-indexer
```

master

-

nginx

```
nginx
```

v1.28.0

-

notifications-processor

```
notifications-processor
```

ca37f9e1f7604a90ccefce99c97e9301ef9ec262

Yes

overseer

```
overseer
```

2f1113b6fe7c44e72b8c8e05a473173e89c3ab9e

Yes

renderer-processor

```
renderer-processor
```

254d9fcfdf64ca2d94ad87a7b87c1bef2a2d6193

Yes

roleman

```
roleman
```

dd9549e231da052a3f80034685b6c6ba37cf6542

Yes

scheduler

```
scheduler
```

35b2ef7-20251121

Yes

trackman

```
trackman
```

2.24.2

Yes

trigger

```
trigger
```

6d9b27d3a13f73fbe0e61c7673f3f51900e7018d

Yes

web-api

```
web-api
```

c4acf877e45178dcf8fbbe4e9328bb5fa97fa3b9

Yes

web-streamer

```
web-streamer
```

21451cc0ebb9a2a8d2d4b0412e9c76b84545bf0d

Yes

web-zone

```
web-zone
```

d903a9460ed996a4e27184fa74768d5697726e15

Yes

zone-assets-server

```
zone-assets-server
```

d903a9460ed996a4e27184fa74768d5697726e15

Yes

## hashtag Public-facing changes

#### hashtag New Gmail app

We've released a new Gmail arrow-up-right app, that simplifies the setup and offers new module enhancements:

- Easier setup : Unlike Gmail (Legacy) arrow-up-right , it doesn't require creating a Google Cloud Platform project and OAuth credentials for personal Gmail accounts.
- New modules and improvements : Additional parameters in existing modules plus new modules like Reply to an Email , Search Emails , Get an Email , Send a Draft Email , List Email Attachments a nd Media and Make an API Call .

Easier setup : Unlike Gmail (Legacy) arrow-up-right , it doesn't require creating a Google Cloud Platform project and OAuth credentials for personal Gmail accounts.

New modules and improvements : Additional parameters in existing modules plus new modules like Reply to an Email , Search Emails , Get an Email , Send a Draft Email , List Email Attachments a nd Media and Make an API Call .

Learn more in our technical documentation: Gmail app arrow-up-right

#### hashtag Make AI Web Search

We've released Make AI Web Search, a built-in app that lets you search and get real-time information directly in your scenarios. You can run all web searches securely within Make, without relying on external integrations.

Use the Generate a response module to run a web search based on your prompt. You can select whether you want the AI's text response to be parsed as JSON and apply location-based filters. These options give you more control over the search context and the format of the AI-generated output, making the results easier to use in the rest of your scenario.

Learn more in our technical documentation: Make AI Web Search app arrow-up-right

#### hashtag New HTTP app

The new HTTP app is now available, giving you a simpler way to create powerful, secure, and scalable integrations with any API:

- Easier setup : An updated UI simplifies the workflow with HTTP requests.
- Reusable data structures for request bodies : A new data structure editor helps to ensure valid JSON requests and enables data reuse across scenarios.
- New built-in pagination : A new capability allows you to configure pagination directly within the module and retrieve large data sets in a structured, manageable way.
- Full compliance with HTTP standards : A new app is fully compliant with current HTTP standards and allows following redirects with 3xx status codes.
- Secure keychain storage and management : A secure keychain stores and encrypts API keys and tokens in one place, allowing simple rotation of keys and credentials management independently of scenarios.
- Secure connections with Mutual TLS and proxy : Support for Mutual TLS and configurable proxy settings enables encrypted and high-security connections.

Easier setup : An updated UI simplifies the workflow with HTTP requests.

Reusable data structures for request bodies : A new data structure editor helps to ensure valid JSON requests and enables data reuse across scenarios.

New built-in pagination : A new capability allows you to configure pagination directly within the module and retrieve large data sets in a structured, manageable way.

Full compliance with HTTP standards : A new app is fully compliant with current HTTP standards and allows following redirects with 3xx status codes.

Secure keychain storage and management : A secure keychain stores and encrypts API keys and tokens in one place, allowing simple rotation of keys and credentials management independently of scenarios.

Secure connections with Mutual TLS and proxy : Support for Mutual TLS and configurable proxy settings enables encrypted and high-security connections.

#### hashtag LinkedIn Events

We've released a new LinkedIn app that allows users and organizations create and manage virtual or in-person events directly in Make.

Learn more in our technical documentation: LinkedIn Events app arrow-up-right

#### hashtag Updated apps

- OpenAI (ChatGPT, Sora, DALL-E, Whisper) arrow-up-right — We’ve added new Video modules that let you generate and remix videos with Sora, track new video jobs, list and delete videos, and retrieve detailed video information. Module names were also refined for better clarity.
- MCP Client arrow-up-right — We've added Execute an action with AI , a new MCP Client module that uses AI to complete tasks with MCP tools. Select which tools the AI can access, describe the task, and the module automatically identifies and executes the right tool. Additionally, the list of verified remote MCP servers has been expanded. This update lets you securely connect to more trusted remote servers directly.
- Notion arrow-up-right — The Internal connection type now works in all Notion modules that were missing it. You can now use this connection type in any Notion module.
- Runway arrow-up-right — We've added a new Generate a Video from a Video module that allows you to create an AI-generated video using an existing video and a text prompt.
- Virtuagym arrow-up-right — The Virtuagym connection process is now simpler. You no longer need to provide your username and password when creating a new connection.
- Parseur arrow-up-right — We've added a new trigger module, Watch Process Failed , that triggers when a process doesn’t complete successfully.
- Frame.io arrow-up-right — A new version of the Frame.io app is now live. This new version ensures seamless integration and up-to-date functionality.
- Tellent Recruitee arrow-up-right — The Recruitee app has been renamed to Tellent Recruitee reflect the platform’s rebranding.

OpenAI (ChatGPT, Sora, DALL-E, Whisper) arrow-up-right — We’ve added new Video modules that let you generate and remix videos with Sora, track new video jobs, list and delete videos, and retrieve detailed video information. Module names were also refined for better clarity.

MCP Client arrow-up-right — We've added Execute an action with AI , a new MCP Client module that uses AI to complete tasks with MCP tools. Select which tools the AI can access, describe the task, and the module automatically identifies and executes the right tool. Additionally, the list of verified remote MCP servers has been expanded. This update lets you securely connect to more trusted remote servers directly.

Notion arrow-up-right — The Internal connection type now works in all Notion modules that were missing it. You can now use this connection type in any Notion module.

Runway arrow-up-right — We've added a new Generate a Video from a Video module that allows you to create an AI-generated video using an existing video and a text prompt.

Virtuagym arrow-up-right — The Virtuagym connection process is now simpler. You no longer need to provide your username and password when creating a new connection.

Parseur arrow-up-right — We've added a new trigger module, Watch Process Failed , that triggers when a process doesn’t complete successfully.

Frame.io arrow-up-right — A new version of the Frame.io app is now live. This new version ensures seamless integration and up-to-date functionality.

Tellent Recruitee arrow-up-right — The Recruitee app has been renamed to Tellent Recruitee reflect the platform’s rebranding.

Last updated 4 months ago
