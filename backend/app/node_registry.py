...
NODE_DEFINITIONS: list[dict[str, Any]] = [
...

    {
        ...
        "category": "AI / MCP",
        "description": "Connect to an external MCP server and use its tools. Attach to an AI Agent to give it access to MCP-exposed tools.",
        "icon": "plug",
        "color": "#3B82F6",
        "supports_branches": False,
        "fields": [
            {"key": "transport", "label": "Server Transport", "type": "select", "required": True, "default": "sse",
             "options": [
                 {"label": "SSE (Server-Sent Events)", "value": "sse"},
                 {"label": "Streamable HTTP", "value": "streamable_http"},
             ]},
            {"key": "endpoint_url", "label": "MCP Endpoint URL", "type": "string", "required": True,
             "help": "URL of the external MCP server (e.g. https://mcp.notion.com/mcp)"},
            {"key": "authentication", "label": "Authentication", "type": "select", "required": False, "default": "none",
             "options": [
                 {"label": "None", "value": "none"},
            ...
            {"key": "auth_token", "label": "Token / API Key", "type": "string", "required": False,
             "show_when": {"field": "authentication", "equals": "bearer"},
             "help": "Bearer token for authentication"},
            {"key": "tool_selection", "label": "Tools to Include", "type": "select", "required": False, "default": "all",
             "options": [
                 {"label": "All Tools", "value": "all"},
                 {"label": "Selected Only", "value": "selected"},
                 {"label": "All Except", "value": "except"},
             ]},
            {"key": "selected_tools", "label": "Tool Names", "type": "string", "required": False,
             "show_when": {"field": "tool_selection", "equals": "selected"},
    ...
    },
    {
        ...
        "category": "AI / MCP",
        "description": "Expose this workflow as an MCP server. External MCP clients can discover and call tools defined in this workflow.",
        "icon": "server",
        "color": "#8B5CF6",
        "supports_branches": False,
        "fields": [
            {"key": "path", "label": "MCP URL Path", "type": "string", "required": False, "default": "/mcp/auto",
             "help": "Custom URL path for this MCP server endpoint"},
            {"key": "authentication", "label": "Authentication", "type": "select", "required": False, "default": "none",
             "options": [
                 {"label": "None", "value": "none"},
            ...
            {"key": "auth_token", "label": "Expected Token", "type": "string", "required": False,
             "show_when": {"field": "authentication", "equals": "bearer"}},
            {"key": "tools_description", "label": "Server Description", "type": "textarea", "required": False,
             "help": "Description shown to MCP clients discovering this server"},
        ],
    },
    {
        "type": "mcp_client",
        "label": "MCP Client",
        "category": "AI / MCP",
        "description": "Call a specific tool on an external MCP server as a regular workflow step. Select the tool and provide parameters.",
        "icon": "terminal",
        "color": "#06B6D4",
        "supports_branches": False,
        "fields": [
            ...
            {"key": "auth_token", "label": "Token", "type": "string", "required": False,
             "show_when": {"field": "authentication", "equals": "bearer"}},
            {"key": "tool_name", "label": "Tool Name", "type": "string", "required": True,
             "help": "Name of the MCP tool to call"},
            {"key": "input_mode", "label": "Input Mode", "type": "select", "required": False, "default": "manual",
             "options": [
                 {"label": "Manual", "value": "manual"},
                 {"label": "JSON", "value": "json"},
             ]},
            {"key": "tool_parameters", "label": "Tool Parameters (JSON)", "type": "code", "required": False,
             "show_when": {"field": "input_mode", "equals": "json"}},
            {"key": "timeout", "label": "Timeout (ms)", "type": "number", "required": False, "default": 30000},
        ],
    },
...

    {
        ...
        "description": "Pause workflow execution and wait for human approval before proceeding. Use for HITL (Human-in-the-Loop) gates in agent workflows.",
        "icon": "user-check",
        "color": "#F59E0B",
        "supports_branches": True,
        "fields": [
            {"key": "approval_prompt", "label": "Approval Prompt", "type": "textarea", "required": True,
             "default": "Please review and approve this action.",
             "help": "Message shown to the human reviewer"},
            {"key": "context_fields", "label": "Context Fields", "type": "string", "required": False,
             "help": "Comma-separated field names to show reviewer for context"},
            {"key": "timeout_minutes", "label": "Timeout (minutes)", "type": "number", "required": False, "default": 60,
             "help": "Auto-reject if no response within timeout. 0 = no timeout."},
            {"key": "timeout_action", "label": "On Timeout", "type": "select", "required": False, "default": "reject",
             "options": [
                 {"label": "Reject (stop)", "value": "reject"},
                 {"label": "Approve (continue)", "value": "approve"},
                 {"label": "Skip to fallback", "value": "fallback"},
             ]},
            {"key": "notify_channels", "label": "Notification Channels", "type": "select", "required": False, "default": "in_app",
             "options": [
                 {"label": "In-App Only", "value": "in_app"},
                 {"label": "Email + In-App", "value": "email"},
                 {"label": "Slack + In-App", "value": "slack"},
             ]},
        ],
    },
    {
        ...
        "category": "AI / Advanced",
        "description": "Test agent outputs against golden datasets. Compare agent responses to expected outputs using various metrics.",
        "icon": "clipboard-check",
        "color": "#10B981",
        "supports_branches": False,
        "fields": [
            {"key": "evaluation_mode", "label": "Evaluation Mode", "type": "select", "required": True, "default": "comparison",
             "options": [
                 {"label": "Golden Dataset Comparison", "value": "comparison"},
                 {"label": "LLM-as-Judge", "value": "llm_judge"},
                 {"label": "Regex Match", "value": "regex"},
                 {"label": "Contains Keywords", "value": "keywords"},
             ]},
            {"key": "agent_output_field", "label": "Agent Output Field", "type": "string", "required": True, "default": "answer",
             "help": "Field containing the agent's response"},
            {"key": "expected_output_field", "label": "Expected Output Field", "type": "string", "required": False,
             "show_when": {"field": "evaluation_mode", "equals": "comparison"},
             "help": "Field containing the expected/golden answer"},
            {"key": "similarity_threshold", "label": "Similarity Threshold", "type": "number", "required": False, "default": 0.8,
             "show_when": {"field": "evaluation_mode", "equals": "comparison"},
             "help": "Minimum similarity score (0-1) to pass"},
            {"key": "judge_prompt", "label": "Judge System Prompt", "type": "textarea", "required": False,
             "show_when": {"field": "evaluation_mode", "equals": "llm_judge"},
             "help": "System prompt for the LLM judge"},
            {"key": "pattern", "label": "Pattern", "type": "string", "required": False,
             "show_when": {"field": "evaluation_mode", "equals": "regex"}},
            {"key": "keywords", "label": "Keywords", "type": "string", "required": False,
             "show_when": {"field": "evaluation_mode", "equals": "keywords"},
             "help": "Comma-separated keywords to check for"},
            {"key": "output_metrics", "label": "Output Metrics", "type": "boolean", "required": False, "default": True,
    ...
    },
    {
        ...
        "label": "Set",
        "category": "Data Transformation",
        "description": "Add, modify, or remove fields on each item. Equivalent to n8n's 'Edit Fields' node.",
        "icon": "edit",
        "supports_branches": False,
        "fields": [
            {
                "key": "source",
                "label": "Input Items (field or blank for auto)",
                "type": "text",
                "required": False,
                "default": "",
                "help": "Leave blank to use items from the previous step.",
            },
            {
                "key": "assignments",
                "label": "Field Assignments (JSON array)",
                "type": "code",
                "required": True,
                "default": '[{"key": "newField", "value": "{{$item.existingField}}", "operation": "set"}]',
                "help": "Array of {key, value, operation}. operation: 'set' (default) or 'remove'.",
            },
        ],
    },
    {
        ...
        "label": "Date & Time",
        "category": "Data Transformation",
        "description": "Perform date and time operations: get current time, format, add/subtract, or calculate differences.",
        "icon": "calendar",
        "supports_branches": False,
        "fields": [
            {
                ...
                "options": [
                    ...
                    {"value": "add", "label": "Add/Subtract Duration"},
                    {"value": "diff", "label": "Calculate Difference"},
                ],
                "help": "Select which datetime operation to perform.",
            },
            {
                "key": "timezone",
                "label": "Timezone",
                ...
                "required": False,
                "default": "UTC",
                "help": "IANA timezone name, e.g. 'America/New_York'. Used for 'now' operation.",
            },
            {
                "key": "format",
                "label": "Output Format",
                ...
                "required": False,
                "default": "iso",
                "help": "Use 'iso' for ISO-8601, or a strftime format like '%Y-%m-%d %H:%M:%S'.",
            },
            {
                "key": "input",
                "label": "Input Date (for format/add operations)",
                "type": "text",
                "required": False,
                "default": "",
                "help": "ISO date string or expression. Used by 'format' and 'add' operations.",
            },
            {
                "key": "days",
                ...
                "required": False,
                "default": 0,
                "help": "Number of days to add (positive) or subtract (negative). Used by 'add' operation.",
            },
            {
                "key": "hours",
                ...
                "required": False,
                "default": 0,
                "help": "Number of hours to add (positive) or subtract (negative).",
            },
            {
                "key": "date_a",
                "label": "Date A (earlier date for diff)",
                "type": "text",
                "required": False,
                "default": "",
                "help": "ISO date string or expression for the earlier date.",
            },
            {
                "key": "date_b",
                "label": "Date B (later date for diff)",
                "type": "text",
                "required": False,
                "default": "",
                "help": "ISO date string or expression for the later date.",
            },
            {
                "key": "unit",
                "label": "Diff Unit",
                ...
                "help": "Unit for the difference calculation.",
            },
        ],
    },
    {
        ...
        "label": "HTTP Request",
        "category": "Core",
        "description": "Make HTTP requests to any URL. Supports GET, POST, PUT, PATCH, DELETE with JSON/form/raw body, custom headers, and basic/bearer auth.",
        "icon": "globe",
        "supports_branches": False,
        "fields": [
            {
                ...
                "required": True,
                "default": "",
                "help": "The URL to request. Supports expressions like {{$json.url}}.",
            },
            {
                "key": "method",
                "label": "Method",
            ...
            },
            {
                ...
                "help": "JSON object of request headers. Keys and values support expressions.",
            },
            {
                "key": "body_type",
                "label": "Body Type",
                ...
                "help": "Type of request body to send.",
            },
            {
                "key": "body",
                "label": "Request Body",
                ...
                "required": False,
                "default": '{"key": "value"}',
                "help": "Body content. JSON object for JSON/form, string for raw. Supports expressions.",
            },
            {
                "key": "auth_type",
                "label": "Authentication",
                ...
                "help": "Authentication method for the request.",
            },
            {
                "key": "auth_username",
            ...
            },
            {
                ...
                "required": False,
                "default": "",
                "help": "Password for Basic authentication. Use vault reference: {{vault.credential.mySecret}}.",
            },
            {
                "key": "auth_token",
                "label": "Bearer Token",
                ...
                "required": False,
                "default": "",
                "help": "Bearer token value. Use vault reference: {{vault.credential.myToken}}.",
            },
            {
                "key": "response_type",
                "label": "Response Format",
                ...
                "default": "auto",
                "options": [
                    {"value": "auto", "label": "Auto-detect (JSON or text)"},
                    {"value": "json", "label": "JSON"},
                    {"value": "text", "label": "Text"},
                ],
                "help": "How to parse the response body.",
            },
            {
                "key": "raise_on_error",
                "label": "Raise on HTTP Error",
                "type": "boolean",
                "required": False,
                "default": True,
                "help": "If enabled, non-2xx responses throw an error and halt execution.",
            },
            {
                "key": "timeout_seconds",
                "label": "Timeout (seconds)",
                ...
                "required": False,
                "default": 30,
                "help": "Maximum time to wait for a response before timing out.",
            },
        ],
    },
    {
        ...
        "label": "Code",
        "category": "Core",
        "description": "Run a custom Python code snippet. Access items via the 'items' variable, set 'output' to return data to the next node.",
        "icon": "code",
        "supports_branches": False,
        "fields": [
            {
                ...
                "type": "code",
                "required": True,
                "default": "# Access input items\n# items = [{'name': 'Alice'}, {'name': 'Bob'}]\n\n# Process and set output\noutput = [{'greeting': f\"Hello {item['name']}\"} for item in items]\n",
                "help": "Python code to execute. Available: items (input list), payload (trigger data), json, re. Set 'output' to pass data to the next node.",
            },
        ],
    },
]
