# Credential overwrites | n8n Docs

Source: https://docs.n8n.io/hosting/configuration/credential-overwrites
Lastmod: 2026-04-14
...
## Using the REST API[#](#using-the-rest-api "Permanent link")
...
1. Set `CREDENTIALS_OVERWRITE_ENDPOINT` to the path where the endpoint should be available:
   ...
   |  |  |
   | --- | --- |
   | ``` 1 ``` | ``` export CREDENTIALS_OVERWRITE_ENDPOINT=send-credentials ``` |
...
3. Send the file to your n8n instance:
   ...
   |  |  |
   | --- | --- |
   | ``` 1 ``` | ``` curl -H "Content-Type: application/json" -H "Authorization: Bearer secure-token" --data @oauth-credentials.json http://localhost:5678/send-credentials ``` |
...
|  |  |
| --- | --- |
| ``` 1 ``` | ``` export CREDENTIALS_OVERWRITE_PERSISTENCE=true ``` |
