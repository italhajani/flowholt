...
export async function GET(_: NextRequest, context: RouteContext) {
  ...
  const simulation = simulateWorkflowGraph(workflow.graph);
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const graphToSimulate = isGraphCandidate(draftGraph) ? draftGraph : workflow.graph;

  const validation = validateWorkflowGraph(graphToSimulate);
  const simulation = simulateWorkflowGraph(graphToSimulate);
