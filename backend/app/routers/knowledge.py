...
@router.get(f"{settings.api_prefix}/knowledge", response_model=list[KnowledgeBaseSummary])
...
@router.post(f"{settings.api_prefix}/knowledge", response_model=KnowledgeBaseSummary, status_code=201)
...
@router.get(f"{settings.api_prefix}/knowledge/{{kb_id}}", response_model=KnowledgeBaseSummary)
...
@router.put(f"{settings.api_prefix}/knowledge/{{kb_id}}", response_model=KnowledgeBaseSummary)
...
@router.delete(f"{settings.api_prefix}/knowledge/{{kb_id}}")
async def api_delete_knowledge_base(kb_id: str) -> Response:
...
@router.get(f"{settings.api_prefix}/knowledge/{{kb_id}}/documents", response_model=list[KnowledgeDocumentOut])
async def api_list_documents(kb_id: str):
    return list_knowledge_documents(kb_id)


@router.post(f"{settings.api_prefix}/knowledge/{{kb_id}}/documents", response_model=KnowledgeDocumentOut, status_code=201)
...
@router.delete(f"{settings.api_prefix}/knowledge/documents/{{doc_id}}")
async def api_delete_document(doc_id: str) -> Response:
...
@router.post(f"{settings.api_prefix}/knowledge/{{kb_id}}/search", response_model=list[KnowledgeSearchResult])
