...
# Hosting n8n on Google Cloud Run[#](#hosting-n8n-on-google-cloud-run "Permanent link")
...
## Before you begin: get a Google Cloud project[#](#before-you-begin-get-a-google-cloud-project "Permanent link")
...
## Easy mode[#](#easy-mode "Permanent link")
...
|  |  |
| --- | --- |
| ``` 1 2 3 4 5 6 7 8 ``` | ``` gcloud run deploy n8n \     --image=n8nio/n8n \     --region=us-west1 \     --allow-unauthenticated \     --port=5678 \     --no-cpu-throttling \     --memory=2Gi \     --set-env-vars="N8N_ENDPOINT_HEALTH=health" ``` |
...
|  |  |
| --- | --- |
| ``` 1 2 3 4 5 6 7 8 9 ``` | ``` gcloud run deploy n8n \     --image=n8nio/n8n \     --region=us-west1 \     --allow-unauthenticated \     --port=5678 \     --no-cpu-throttling \     --memory=2Gi \     --scaling=1 \     --set-env-vars="N8N_ENDPOINT_HEALTH=health" ``` |
...
## Durable mode[#](#durable-mode "Permanent link")
...
## Enable APIs and set env vars[#](#enable-apis-and-set-env-vars "Permanent link")
...
|  |  |
| --- | --- |
| ``` 1 2 3 4 5 6 ``` | ``` ## You may need to login first gcloud auth login  gcloud services enable run.googleapis.com gcloud services enable sqladmin.googleapis.com gcloud services enable secretmanager.googleapis.com ``` |

You'll also want to set some environment variables for the remainder of these instructions:

|  |  |
| --- | --- |
| ``` 1 2 ``` | ``` export PROJECT_ID=your-project export REGION=region-where-you-want-this-deployed ``` |

## Setup your Postgres database[#](#setup-your-postgres-database "Permanent link")
...
|  |  |
| --- | --- |
| ``` 1 2 3 4 5 6 7 8 9 ``` | ``` gcloud sql instances create n8n-db \     --database-version=POSTGRES_13 \     --tier=db-f1-micro \     --region=$REGION \     --root-password="change-this-password" \     --storage-size=10GB \     --availability-type=ZONAL \     --no-backup \     --storage-type=HDD ``` |

Once complete, you can add the database that n8n will use:

|  |  |
| --- | --- |
| ``` 1 ``` | ``` gcloud sql databases create n8n --instance=n8n-db ``` |

Create the DB user for n8n (change the password value, of course):

|  |  |
| --- | --- |
| ``` 1 2 3 ``` | ``` gcloud sql users create n8n-user \     --instance=n8n-db \     --password="change-this-password" ``` |
...
## Store sensitive data in Secret Manager[#](#store-sensitive-data-in-secret-manager "Permanent link")
...
|  |  |
| --- | --- |
| ``` 1 2 3 ``` | ``` gcloud secrets create n8n-db-password \     --data-file=/your/password/file \     --replication-policy="automatic" ``` |

Create an encryption key (you can use your own, this example generates a random one):

|  |  |
| --- | --- |
| ``` 1 ``` | ``` openssl rand -base64 -out my-encryption-key 42 ``` |

Create a secret for this encryption key (replace "my-encryption-key" if you are supplying your own):

|  |  |
| --- | --- |
| ``` 1 2 3 ``` | ``` gcloud secrets create n8n-encryption-key \     --data-file=my-encryption-key \     --replication-policy="automatic" ``` |
...
## Create a service account for Cloud Run[#](#create-a-service-account-for-cloud-run "Permanent link")
...
## (Optional) Enabling Google Workspace services as n8n tools[#](#optional-enabling-google-workspace-services-as-n8n-tools "Permanent link")
...
|  |  |
| --- | --- |
| ``` 1 2 3 4 5 6 7 ``` | ``` ## Enable whichever APIs you need ## Note: If you want Sheets/Docs, it's not enough to just enable Drive; these services each have their own API gcloud services enable gmail.googleapis.com gcloud services enable drive.googleapis.com gcloud services enable sheets.googleapis.com gcloud services enable docs.googleapis.com gcloud services enable calendar-json.googleapis.com ``` |

Re-deploy n8n on Cloud Run with the necessary OAuth callback URLs as environment variables:

|  |  |
| --- | --- |
| ``` 1 2 3 4 5 6 ``` | ``` export SERVICE_URL="your-n8n-service-URL" ## e.g. https://n8n-12345678.us-west1.run.app  gcloud run services update n8n \     --region=$REGION \     --update-env-vars="N8N_HOST=$(echo $SERVICE_URL | sed 's/https:\/\///'),WEBHOOK_URL=$SERVICE_URL,N8N_EDITOR_BASE_URL=$SERVICE_URL" ``` |
