...
### 1Password[#](#1password "Permanent link")
...
### AWS Secrets Manager[#](#aws-secrets-manager "Permanent link")
...
### Azure Key Vault[#](#azure-key-vault "Permanent link")
...
### GCP Secrets Manager[#](#gcp-secrets-manager "Permanent link")
...
### HashiCorp Vault[#](#hashicorp-vault "Permanent link")
...
#### Manual KV mount configuration[#](#manual-kv-mount-configuration "Permanent link")
...
|  |  |
| --- | --- |
| ``` 1 2 3 4 5 6 7 ``` | ``` # Read and list secrets at the "secret/" KV v2 mount path "secret/data/*" {   capabilities = ["read"] } path "secret/metadata/*" {   capabilities = ["read", "list"] } ``` |

For KV v1, you only need a single policy path:

|  |  |
| --- | --- |
| ``` 1 2 3 4 ``` | ``` # Read and list secrets at the "kv/" KV v1 mount path "kv/*" {   capabilities = ["read", "list"] } ``` |

## Share vault[#](#share-vault "Permanent link")
...
### Access for project roles[#](#access-for-project-roles "Permanent link")
...
### Custom roles[#](#custom-roles "Permanent link")
...
### Secrets don't resolve in production[#](#secrets-dont-resolve-in-production "Permanent link")
